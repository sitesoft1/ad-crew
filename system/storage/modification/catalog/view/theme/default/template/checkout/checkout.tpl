...
$(document).delegate('#button-shipping-method', 'click', function() {
                    				...
                    				var pvz_id = false, pvz_name = false;

		    pvz_id = $(document).find('.pvzcode').val();
		    pvz_name = $(document).find('#pvzname').val();

		    if (pvz_id && pvz_name) {
		        $.ajax({
		            url: 'index.php?route=checkout/shipping_method/save_pvz',
		            type: 'post',
		            data: {
		                pvz_id: pvz_id,
		                pvz_name: pvz_name
